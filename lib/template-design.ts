...
const textSegmentStyleSchema = z.object({
...
export function designHasBinding(design: TemplateDesign | null | undefined, binding: string): boolean {
  if (!design || !binding) return false;
  const target = binding.trim();
  if (!target) return false;

  const checkElements = (elements: DesignElement[] | undefined): boolean => {
    if (!elements) return false;
    for (const element of elements) {
      if (element.visibility?.binding === target) return true;
      if (element.type === "text") {
        if (element.binding === target) return true;
        if (element.parts?.some((part: TextPart) => part.type === "binding" && part.field === target)) return true;
      }
      if (element.type === "stack" && checkElements(element.items)) return true;
    }
    return false;
  };

  return checkElements(design.front) || checkElements(design.back);
}
